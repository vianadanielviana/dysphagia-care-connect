-- =====================================================
-- FIX PACIENTES_SAFE VIEW SECURITY VULNERABILITIES
-- =====================================================
...
-- 2. Create secure is_system_admin function (removes hardcoded email)
...
-- 4. Recreate the pacientes_safe view with proper security
...
SELECT 
    ...
    -- Birth date - less sensitive but still controlled
    CASE
        WHEN (
          public.is_authorized_for_patient(p.id) 
          AND EXISTS ( SELECT 1 FROM public.profiles pr
    ...
    -- System fields - always show (non-sensitive)
    p.status,
    p.professional_id,
    p.caregiver_id,
