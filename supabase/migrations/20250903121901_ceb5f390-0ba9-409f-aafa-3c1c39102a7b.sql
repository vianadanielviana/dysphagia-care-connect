...
-- Add user_id column to cadastro table for proper user isolation
ALTER TABLE public.cadastro ADD COLUMN user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Update existing records to be owned by admin user (to prevent data loss)
-- Get the admin user ID based on email from profiles table
