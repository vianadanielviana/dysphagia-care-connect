-- Enhanced Security Implementation for Patient Medical Records
...
-- Create a function to get masked patient data based on access level
CREATE OR REPLACE FUNCTION get_patient_secure_masked(patient_uuid UUID)
RETURNS TABLE(
...
BEGIN
  ...
  INSERT INTO patient_access_log (
    ...
    auth.uid(), patient_uuid, 'SECURE_DATA_ACCESS_ATTEMPT',
    (SELECT tipo_usuario FROM profiles WHERE id = auth.uid()),
    jsonb_build_object(
      'access_level', access_level,
      'timestamp', now(),
      'ip_address', inet_client_addr(),
      'function', 'get_patient_secure_masked'
    )
  );
...
-- Add comprehensive audit trigger for all patient data access
...
BEGIN
  ...
  INSERT INTO patient_access_log (
    ...
    CASE 
      WHEN TG_OP = 'SELECT' THEN 'PATIENT_DATA_VIEWED'
...
CREATE TRIGGER comprehensive_patient_audit_trigger
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON pacientes
  FOR EACH ROW EXECUTE FUNCTION comprehensive_patient_audit();

-- Create a function to grant temporary access to patients
...
-- Create a function to revoke patient access
...
AS $$
BEGIN
  -- Only admins and assigned professionals can revoke access
  IF NOT (is_system_admin_secure() OR 
          EXISTS (SELECT 1 FROM pacientes WHERE id = target_patient_id AND professional_id = auth.uid())) THEN
    RAISE EXCEPTION 'Insufficient privileges to revoke patient access';
  END IF;
  
  -- Revoke permission
  UPDATE patient_access_permissions
  SET is_active = false, updated_at = now()
  WHERE user_id = target_user_id AND patient_id = target_patient_id;
  
  -- Log the access revocation
  INSERT INTO patient_access_log (
    user_id, patient_id, action, user_type, metadata
  ) VALUES (
    auth.uid(), target_patient_id, 'ACCESS_REVOKED',
    (SELECT tipo_usuario FROM profiles WHERE id = auth.uid()),
    jsonb_build_object(
      'target_user_id', target_user_id,
      'timestamp', now()
