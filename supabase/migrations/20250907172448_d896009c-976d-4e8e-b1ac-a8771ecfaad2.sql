-- COMPREHENSIVE SECURITY FIXES - PHASE 1: CRITICAL DATA PROTECTION (CORRECTED)
-- Fix critical security vulnerabilities identified in security review

-- 1. FIX USERS_CONTACT_SAFE ACCESS CONTROL
-- Drop the view and replace with a secure function that handles access control
DROP VIEW IF EXISTS public.users_contact_safe;

-- Create a secure function to get user contact info with proper access control
...
AS $$
BEGIN
  -- Only allow healthcare professionals to access this data
  IF NOT EXISTS (
  ...
  -- Return masked contact data for authorized users
  RETURN QUERY
  SELECT 
    u.id,
    u.name,
    ...
    END as phone,
    u.user_type,
    -- Mask registration number: show last 2 chars
    CASE 
...
GRANT EXECUTE ON FUNCTION public.get_users_contact_safe() TO authenticated;
...
-- Update n8n_chat_histories policies
...
DROP TRIGGER IF EXISTS audit_patient_access_enhanced_trigger ON public.pacientes;
...
ADD CONSTRAINT IF NOT EXISTS check_valid_status CHECK (status IN ('ativo', 'inativo', 'pendente'));
...
DROP TRIGGER IF EXISTS detect_suspicious_activity_trigger ON public.patient_access_log;
...
-- Add comments for security audit trail
...
COMMENT ON FUNCTION public.get_users_contact_safe() IS 
'Secure function to get masked user contact info with proper access control.';
