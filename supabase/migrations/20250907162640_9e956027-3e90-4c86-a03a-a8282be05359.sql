-- =====================================================
-- CORRECTED SECURITY FIXES FOR PATIENT DATA PROTECTION
-- =====================================================
...
-- 2. CORRECTED logging function with proper OLD/NEW handling
...
AS $$
  SELECT EXISTS (
    ...
    WHERE p.id = patient_uuid
      ...
      AND (
        -- Professional assigned to patient (fixed typo)
...
-- 4. Drop existing policies and create more secure ones
...
AS $$
BEGIN
  -- Validate that professional exists and is approved (if set)
  ...
  -- Validate that caregiver exists and is approved (if set)
